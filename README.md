...
11. Configures open source AI-powered development tools, with inference routed through [Venice](https://venice.ai) for end-to-end encryption between client and inference:
    ...
    - Three Claude Code wrappers in `~/.local/bin/`, all sharing the per-session worktree helper at `bin/claude-create-worktree`:
      ...
      - `claude-private` — routes through `ccr` to Venice's current `default_code` model (E2EE between client and Venice; Anthropic only ships the CLI). `bin/lib/venice-resolve.bash` queries Venice at install time and caches the resolved id in `~/.cache/claude-wrappers/default_code`, so the wrapper auto-tracks Venice's catalog as models rotate. Set `CLAUDE_PRIVATE_THINK=1` to escalate to `claude-opus-4-7` for heavy reasoning. Runs on the host (ccr listens on host loopback) but still gets a fresh worktree.
...
13. Project plumbing for AI agents in this repo:
    ...
    - `.claude/hooks/monitor.bash` — **AI safety monitor** implementing the [AI control](https://arxiv.org/abs/2312.06942) pattern: a cheap, trusted model (qwen3-coder-480b via Venice E2EE, or Haiku) reviews every tool call before the primary model can execute it. Logs decisions to `~/.cache/claude-monitor/monitor.jsonl`. Overhead: ~250–700 tokens per monitored call (~$0.05 per 100 calls on Venice). Skips Read calls. Disable with `MONITOR_DISABLED=1`.
...
- **Container.** `.devcontainer/` is the [Anthropic reference container](https://github.com/anthropics/claude-code/tree/main/.devcontainer) plus `fish`/`pnpm`/`python3` and a few extra approved domains in the egress allowlist. claude-code itself is installed at container creation time (postCreateCommand) via `pnpm add -g --allow-build=@anthropic-ai/claude-code` so the arch-specific native binary always matches the runtime — image build arch no longer matters. The `claude-code-config` named volume persists `/home/node/.claude` across rebuilds; the `claude` wrapper snapshots it before each session for rollback (see above).
- **Worktree per session.** Every `claude` / `claude-private` / `claude-paranoid` invocation lands in a fresh `<repo>/.worktrees/claude-<ts>` worktree, so a bad session is discardable with `git worktree remove`. Skip with `CLAUDE_NO_WORKTREE=1`.
- **Sandbox.** `ai/prompting/settings.json` (symlinked to `~/.claude/settings.json`) enables `sandbox`, denies reads of secret stores (`~/.ssh`, `~/.aws`, `~/.gnupg`, `**/.env*`), denies writes to shell rc files (including fish), and restricts outbound network to an explicit allowlist.
- **Permissions.** Deny rules block shell-out vectors (`curl`, `wget`, `python -c`, `eval`) and exfil destinations (pastebin/transfer.sh/gists). Allow rules pre-approve routine package, test, and git commands. Inside the container the wrapper passes `--dangerously-skip-permissions`; the firewall + bind-mount sandbox + worktree isolation make the permission prompt redundant there.
- **Firewall.** `.devcontainer/init-firewall.bash` runs at container start (`postStartCommand`) and drops all outbound traffic except to a hand-curated allowlist of registries, docs sites, and Anthropic/Venice endpoints.
