...
check_shfmt() {
    ...
    echo -n "Shfmt: "
    local shfmt_targets=(setup.sh bin/*.sh bin/lib/*.sh bin/dotfiles .hooks/*.sh .hooks/pre-push .hooks/pre-commit .hooks/commit-msg .claude/hooks/*.sh .devcontainer/*.sh .bashrc)
...
check_shellcheck() {
    ...
    if [[ "$FIX_MODE" == true ]]; then
        local diff_output
        diff_output=$(shellcheck -e SC2016 --format=diff setup.sh bin/*.sh bin/lib/*.sh bin/dotfiles .hooks/*.sh .hooks/pre-push .hooks/pre-commit .hooks/commit-msg .claude/hooks/*.sh .devcontainer/*.sh 2>/dev/null || true)
        ...
        if shellcheck -e SC2016 setup.sh bin/*.sh bin/lib/*.sh bin/dotfiles .hooks/*.sh .hooks/pre-push .hooks/pre-commit .hooks/commit-msg .claude/hooks/*.sh .devcontainer/*.sh 2>/dev/null &&
